...
void drawstring(float x,float y,float z,char *string) //display page method
{
	char *c;
	glRasterPos3f(x,y,z);
...
float clr(float x) //To display color
{
    float color = x/255;
    return color;
}
void circle(GLdouble rad) //to display sun
{
    GLint points= 50;
...
void lineCirle(){ //yellow horizontal line
    glBegin(GL_LINES);
	for(int i=0;i<300;i++)
	{
		float pi=3.1416;
...
void alltransports_move(int value){ //for singnal
...
void signal1(float x,float y){ //signal box
...
void helpscreen() //2nd display screen
{
...
void frontscreen(void) //!st display screen
{
setFont(GLUT_BITMAP_TIMES_ROMAN_24);
glClearColor(0.5019607843,0.0,0.5019607843,0);
glClear(GL_COLOR_BUFFER_BIT);
glColor3f(1,0,0);
drawstring(-5,7,0.0," TRAFFIC SIGNALS");
glColor3f(1,0.5,0);
drawstring(-4,4,0.0,"Implemented By:");
glColor3f(0,0,0);
drawstring(-1.50,1,0.0,"Sukanya S");
glColor3f(0,0,0);
drawstring(-1.50,-1,0.0,"Prajna S");
glColor3f(1.0,0.0,0.0);
drawstring(-5.0,-4.0,0.0,"Press Enter to start");
glFlush();
}
void display1(){
...
switch(key)
{
    ...
    if(key == 27){ //ESP button ASCII value
        exit(0);
    }
    if(key == 'r'){
        start = false;
...
int main()
{
	...
	glutCreateWindow ("Traffic signal Simulation");
